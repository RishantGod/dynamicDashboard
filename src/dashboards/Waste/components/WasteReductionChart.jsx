...
function ScenarioComparisonChart({ interventionFunding, totalWasteGenerated, enabledInterventions }) {
  const svgRef = useRef();
  const [tooltip, setTooltip] = useState(null);
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
  const [yAxisMetric, setYAxisMetric] = useState('carbon'); // 'carbon' or 'cost'
  ...
  const generateScenarioData = () => {
    ...
    const interventionEffects = {
      foodWaste: { carbonReduction: 0.12, costReduction: 0.10 }, // 12% carbon, 10% cost
      campusReuse: { carbonReduction: 0.08, costReduction: 0.08 },
      recyclingUplift: { carbonReduction: 0.10, costReduction: 0.06 },
      circularProcurement: { carbonReduction: 0.09, costReduction: 0.07 },
      cdMinimisation: { carbonReduction: 0.06, costReduction: 0.04 }
    };
    ...
    Object.keys(enabledInterventions).forEach(intervention => {
      if (enabledInterventions[intervention] && interventionEffects[intervention]) {
        totalCarbonReduction += interventionEffects[intervention].carbonReduction;
        totalCostReduction += interventionEffects[intervention].costReduction;
      }
    });

    // Maximum possible reductions (when all interventions enabled)
    const maxCarbonReduction = 0.45;
    const maxCostReduction = 0.35;

    return years.map(year => {
      const yearIndex = year - 2025;
      const baseWaste = totalWasteGenerated * (1 + yearIndex * 0.015); // 1.5% annual growth baseline
      
      // Business as usual (0% intervention)
      const bauCarbonEmissions = baseWaste * 2.1; // 2.1 tCO₂e per ton
      const bauCost = baseWaste * 120; // €120 per ton

      // Partial intervention (current slider value scaled by enabled interventions)
      const partialReduction = interventionFunding / 100;
      const partialCarbonEmissions = bauCarbonEmissions * (1 - partialReduction * totalCarbonReduction);
      ...
      const fullCarbonEmissions = bauCarbonEmissions * (1 - fullReduction * totalCarbonReduction);
      const fullCost = bauCost * (1 - fullReduction * totalCostReduction);

      return {
        year,
        businessAsUsual: {
          carbon: Math.round(bauCarbonEmissions),
          cost: Math.round(bauCost)
        },
        partial: {
          carbon: Math.round(partialCarbonEmissions),
          cost: Math.round(partialCost)
        },
        fullIntervention: {
          carbon: Math.round(fullCarbonEmissions),
  ...
  useEffect(() => {
    ...
    const width = containerRect.width - margin.left - margin.right - 20; // Use full width minus margins and small buffer
    const height = containerRect.height - margin.top - margin.bottom - 80; // Use full height minus margins and header space
    ...
    g.append('text')
      ...
      .attr('fill', '#27ae60')
      .text(yAxisMetric === 'carbon' ? 'Carbon Emissions (tCO₂e)' : 'Annual Cost (€)');
    ...
    const legend = g.append('g')
      .attr('class', 'legend')
      .attr('transform', `translate(${width / 2 - 250}, ${height + 60})`); // Center horizontally, position below x-axis

    lineConfigs.forEach((config, index) => {
      const legendRow = legend.append('g')
        .attr('transform', `translate(${index * 160}, 0)`); // Arrange horizontally with more spacing
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              cursor: 'pointer',
              background: yAxisMetric === 'carbon' ? '#27ae60' : 'transparent',
              color: yAxisMetric === 'carbon' ? '#fff' : '#666',
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              cursor: 'pointer',
              background: yAxisMetric === 'cost' ? '#27ae60' : 'transparent',
      ...
      {tooltip && (
        <div style={{
          ...
          <div style={{ marginBottom: '4px' }}>
            ...
            {tooltip.metric === 'carbon' 
              ? `${tooltip.businessAsUsual.carbon.toLocaleString()} tCO₂e`
          ...
          <div style={{ marginBottom: '4px' }}>
            ...
            {tooltip.metric === 'carbon' 
              ? `${tooltip.partial.carbon.toLocaleString()} tCO₂e`
              : `€${tooltip.partial.cost.toLocaleString()}`
            }
          </div>
          <div style={{ marginBottom: '4px' }}>
            <span style={{ color: '#27ae60' }}>●</span> <strong>Full Intervention:</strong> {' '}
            {tooltip.metric === 'carbon' 
              ? `${tooltip.fullIntervention.carbon.toLocaleString()} tCO₂e`
